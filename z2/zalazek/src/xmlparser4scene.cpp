...
{
   XMLSize_t i = 0;
  const XMLCh* att=  Attrs.getValue(i);
  const XMLCh* val=  Attrs.getValue(i+1);
  
  char* sAttrsName = xercesc::XMLString::transcode(att);
  char* valName = xercesc::XMLString::transcode(val);
  
  
  if(strcmp(sAttrsName, "Center_x")==0)
    center.x()=std::atoi(valName);
  if(strcmp(sAttrsName, "Center_y")==0)
    center.y()=std::atoi(valName);
  if(strcmp(sAttrsName, "Center_z")==0)
    center.z()=std::atoi(valName);
  if(strcmp(sAttrsName, "Size_x")==0)
    size.x()=std::atoi(valName);
  if(strcmp(sAttrsName, "Size_y")==0)
    size.y()=std::atoi(valName);
  if(strcmp(sAttrsName, "Size_z")==0)
    size.z()=std::atoi(valName);
                                    ...
                                    )
{
  ...
  if(strcmp(sElemName, "Parameter")==0)
